...
public class Main {
    ...
    public static int[] heroesDamage = {20, 15, 15, 10};
    ...
    public static void healthTreat() {
        for (int i = 0; i < heroesHealth.length; i++) {
            boolean medic = heroesAttackType[i].equals("Medic");
            if (heroesHealth[i] <= 100 && heroesHealth[3] > 0 && !medic) {
                heroesHealth[i] = heroesHealth[i] + medicTreat;
                break;
            }
            if (heroesHealth[i] <= 0 && heroesHealth[3] > 0 && !medic) {
